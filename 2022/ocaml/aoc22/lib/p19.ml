...
let assoc_opt default key alist =
  match List.assoc_opt key alist with None -> default | Some x -> x

let read_data ic =
  (* Cheat: it's always the same resources for all inputs *)
  let regexp =
    Str.regexp
      "Blueprint \\(.+\\): Each ore robot costs \\([0-9]+\\) ore[.] Each clay \
       robot costs \\([0-9]+\\) ore[.] Each obsidian robot costs \\([0-9]+\\) \
       ore and \\([0-9]+\\) clay[.] Each geode robot costs \\([0-9]+\\) ore \
       and \\([0-9]+\\) obsidian"
  in
  let rec reader i =
    ...
    | ln ->
        ...
        let el =
          [
            ...
            (Geode, [ (Ore, toint 6); (Obsidian, toint 7) ]);
          ]
        in
        el :: reader (i + 1)
  in
...
let run_solver max_time blueprint =
  ...
  let get_row typ =
    ...
    | alist ->
        Format.sprintf "|%i,%i,%i,%i" (assoc_opt Ore alist)
          (assoc_opt Clay alist) (assoc_opt Obsidian alist)
  ...
  let cmd =
    Format.sprintf
      "%s --solver Chuffed ../../miniZinc/p19.mzn -o %s -D \"max_time=%i\" -D \
       \"robot_costs = [%s%s%s%s|]\""
      minizinc res_filename max_time (get_row Ore) (get_row Clay)
      (get_row Obsidian) (get_row Geode)
  in
  logv cmd;
