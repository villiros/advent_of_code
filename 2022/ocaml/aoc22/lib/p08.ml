...
module Grid = struct
  ...
  let fold_all grid (f : tpos -> 'a -> 'a) acc =
    let edge =
      List.fold_right
        (fun i acc -> { r = i; c = 0 } :: acc)
        (seq 0 (last_index grid))
        []
    in
...
let read_data ic : tgrid =
  let read_row () =
    ...
    | ln ->
        Array.init (String.length ln) (fun i ->
            int_of_char (String.get ln i) - 48)
  in
  let grid =
    let row = read_row () in
...
let parta data =
  ...
  List.iter
    (fun pos ->
      ...
      ignore
        (List.iter
           (fun dir -> ignore (Grid.fold_from data dir row_proc (-1) pos))
           Grid.directions))
    edges;
...
let partb data =
  let score_at pos =
    ...
    List.fold_left
      ...
      1 Grid.directions
  in
  Grid.fold_all data (fun pos acc -> max (score_at pos) acc) 0
