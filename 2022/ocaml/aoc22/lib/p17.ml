...
class grid ?(height = 0) ?(init_contents = []) () =
  ...
  let height =
    ...
    | _ ->
        assert (height = 0);
        List.length init_contents
  in

  let width =
    match init_contents with [] -> 7 | row :: _ -> String.length row
  in
  ...
  object (self)
        ...
        for r = 0 to other#height - 1 do
          for c = 0 to other#width - 1 do
            if other#at r c = rock then (
              if not (self#in_bounds (row + r) (col + c)) then raise Not_found;
              if data.(row + r).(col + c) <> empty then raise Not_found)
          done
        done;
        true
    ...
    method top_lines_state =
      ...
      let row_to_int row =
        Array.fold_left
          (fun acc c -> Int.shift_left acc 1 + if c = rock then 1 else 0)
          0 row
      in
      let result = ref [] in
...
let rec process_piece grid moves piece row col =
  ...
  let new_col = match moves#pop with Left -> col - 1 | Right -> col + 1 in
  let new_col = if grid#intersect piece row new_col then new_col else col in
  if grid#intersect piece (row - 1) new_col then
    process_piece grid moves piece (row - 1) new_col
  else grid#place piece row new_col
...
let parta data = solve data 2022
let partb data = solve data 1000000000000
